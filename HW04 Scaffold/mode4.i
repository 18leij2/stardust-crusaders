...
void setPixel4(int x, int y, unsigned char colorIndex) {
    unsigned short pixelData = videoBuffer[((y) * (240) + (x)) / 2];
...
void drawRect4(int x, int y, int width, int height, volatile unsigned char colorIndex) {
    ...
    for (int i = 0; i < height; i++) {
        if (!(x & 1) && !(width & 1)) {
            if (width > 0) DMANow(3, &color, &videoBuffer[((y + i) * (240) + (x)) / 2], (2 << 23) | width / 2);
        } else if ((x & 1) && (width & 1)) {
            setPixel4(x, y + i, colorIndex);
            if (width > 1) DMANow(3, &color, &videoBuffer[((y + i) * (240) + (x + 1)) / 2], (2 << 23) | (width - 1) / 2);
        } else if (!(x & 1) && (width & 1)) {
            if (width > 1) DMANow(3, &color, &videoBuffer[((y + i) * (240) + (x)) / 2], (2 << 23) | (width - 1) / 2);
            setPixel4(x + width - 1, y + i, colorIndex);
        } else {
            setPixel4(x, y + i, colorIndex);
            if (width > 2) DMANow(3, &color, &videoBuffer[((y + i) * (240) + (x + 1)) / 2], (2 << 23) | (width - 2) / 2);
            setPixel4(x + width - 1, y + i, colorIndex);
        }
    }
}
...
void fillScreen4(volatile unsigned char colorIndex) {
    ...
    DMANow(3, &color, videoBuffer, (2 << 23) | (38400 / 2));
}
